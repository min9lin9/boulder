...
export function checkReleaseEvidenceBundle(
  ...
  if (!parsed.ok) return { status: "fail", issues: parsed.issues };
...
export function releaseEvidenceIssue(code: ReleaseRecoveryCode, message: string): ReleaseEvidenceIssue {
  return { code, message };
}

export function releaseEvidenceParseFailure(code: ReleaseRecoveryCode, message: string): ReleaseEvidenceParseResult {
  return { ok: false, issues: [releaseEvidenceIssue(code, message)] };
}
...
function fail(code: ReleaseRecoveryCode, message: string): ReleaseEvidenceParseResult {
  return releaseEvidenceParseFailure(code, message);
}

function issue(code: ReleaseRecoveryCode, message: string): ReleaseEvidenceIssue {
  return releaseEvidenceIssue(code, message);
}
